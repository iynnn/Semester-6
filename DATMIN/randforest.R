...
train_control <- trainControl(method="cv", number=10) # make fold, beggingnya menggunakan cross validation, menggunakan 

# train the model
model <- train(Class~., data=german, trControl=train_control,
               method="rf")
# semua metode bisa dignakan makae train, 
...
#LEAVE-ONE-OUT
# definisikan training control
train_control <- trainControl(method="LOOCV") # leave one out, sebanyak data nanti
